...
def H03D(k,j):
	...
	for i in range(k):
		...
		if j==2:
			XYZ[i,:]=[0.1+i*0.01,0.000001,0] #j=2
	return(XYZ)
...
def portraitH(k,t,n,j):
	...
	for i in range(len(CI)):
		XY=np.delete(ShIhIm(CI[i],t_l[i],n),j,axis=1)
		x=XY[:,0]
		y=XY[:,1]
		plt.plot(x,y)
...
def portrait3DH(k,t,n,j):
	#6,10000,1 
	t_l=[t]*k
	CI=H03D(k,j)
	ax = plt.axes(projection="3d")
	for i in range(len(CI)):
		XYZ=ShIhIm(CI[i],t_l[i],n)
		x=XYZ[:,0]
		y=XYZ[:,1]
		z=XYZ[:,2]
		ax.plot(x,y,z)
...
def TEMPS(start,n):
	...
	plt.ylabel('IhIm')
	plt.savefig('Temps_Sin_ImIh.png',format='png',bbox_inches='tight')
