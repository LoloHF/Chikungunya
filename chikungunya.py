...
def X03D(k,j):
	...
	for i in range(k):
		if j==0:
			XYZ[4*i,:]=[0,i*200,1200] #j=0
			XYZ[4*i+1,:]=[0,i*200,1]
			XYZ[4*i+2,:]=[0,1200,i*200]
			XYZ[4*i+3,:]=[0,1,i*200] 
		if j==1:
			XYZ[4*i,:]=[i*400,0,1200]
			XYZ[4*i+1,:]=[i*400,0,1] #j=1
			XYZ[4*i+2,:]=[1,0,i*400]
			XYZ[4*i+3,:]=[1200,0,i*400]
		if j==2:
			XYZ[4*i,:]=[i*200,1200,0]
			XYZ[4*i+1,:]=[i*200,1,0]
			XYZ[4*i+2,:]=[1,i*200,0]
			XYZ[4*i+3,:]=[1200,i*200,0]
...
def portrait3DM(k,j,n):
	...
	for c in CI:
		...
		ax.plot(x,y,z)
	#fig = plt.figure()
	#fig.savefig('Portrait3DHumain.png',format='png',bbox_inches='tight')
	ax.set_xlabel('E')
	ax.set_ylabel('L')
	ax.set_zlabel('A')
	

#This function plots in 2D the phase portrait of the transmisison dynamics 
...
#This function plots in 3D the phase portrait of the transmisison dynamics
...
def H03D(k,j):
	...
	for i in range(k):
		if j==0:
			XYZ[i,:]=[0,0.1+i*0.01,0.000001] #j=0
		if j==1:
			XYZ[i,:]=[0.1+i*0.01,0,0.000001] #j=1
		if j==2:
			XYZ[i,:]=[0.1+i*0.01,0.000001,0]
	return(XYZ)



def portrait3DH(k,j,n):
	#6,10000,1 
	CI=H03D(k,j)
	ax = plt.axes(projection="3d")
	for i in CI:
...
def TEMPS(start,n):
	...
	plt.ylabel('Im,Ih')
	plt.savefig('TempsSh.png',format='png',bbox_inches='tight')
